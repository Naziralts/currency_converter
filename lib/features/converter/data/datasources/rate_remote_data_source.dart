...
// Объяснение кода
// RateRemoteDataSource — абстрактный интерфейс для удалённых источников данных:
// Определяет методы getLatestRate и getHistory.
// RateRemoteDataSourceImpl — реализация через внешние API:
// Использует Dio для HTTP-запросов.
// Подключает freecurrencyapi.com для актуального курса.
// Подключает Frankfurter API для истории курсов.
// Особенности getLatestRate:
// Если база и цель совпадают → курс 1.0.
// Для USD берём прямой курс.
// Для других валют → вычисляем кросс-курс через USD.
// Обрабатываем HTTP ошибки и ошибки сети.
// Особенности getHistory:
// Генерируем URL в формате YYYY-MM-DD.
// Преобразуем JSON в список объектов HistoryPoint.
// Сортируем по дате.
// Если нет данных → возвращаем пустой список.
// Обработка ошибок:
// Разделение ошибок: HTTP ошибка, сетевая ошибка, отсутствие данных.
// Позволяет BLoC корректно показывать сообщения пользователю.
